...
class Auth extends BaseController
{
    ...
    {   
        ...
        if (! $this->validate($rules)) {
            ...
            $user = $this->base_model->where('email', $field['email'])->first();
            if ($user['email']) :
                for (;;) {
                    $get_token = random_string('alnum', 32);
                    $cek_token = $this->base_model->getWhere(['token' => $get_token])->getNumRows();
    ...
    public function accountActivation($token)
    {
        ...
        if (!$user) {
            ...
            $data['name'] = $toName;
            $data['text'] = 'Selamat! Akun ' . model('Env')->webName() . ' Anda telah aktif. Sekarang Anda bisa login.';
            ...
            "<script>
                ...
                position: 'top-end',
                icon: 'success',
                title: 'Aktivasi akun berhasil, Silakan login',
                })
            </script>");
        }
    }
